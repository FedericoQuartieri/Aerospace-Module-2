...
presenta \hyfoam~\cite{hystrath}, un solver a due temperature per flussi
ipersonici reagenti, a codice aperto, costruito su
OpenFOAM~\cite{openfoam13}. La
prima parte del lavoro di riferimento verifica il solver su una serie di bagni termici:
...
Il resto della relazione è organizzato così. La
sezione~\ref{sec:lavoro} descrive i due programmi, il modo in cui sono
collegati e il modello come è stato implementato; la
sezione~\ref{sec:risultati} riporta la verifica caso per caso; la
sezione~\ref{sec:discussione} discute che cosa dicono i risultati e da dove
vengono le differenze rimaste; la sezione~\ref{sec:conclusioni} trae le
conclusioni e indica gli sviluppi futuri.
