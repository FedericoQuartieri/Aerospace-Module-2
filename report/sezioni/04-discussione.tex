...
fisica, danno le stesse curve entro lo 0{,}3\% in tutti i casi
tranne l'azoto ad alta temperatura, reazioni comprese. Il confronto con il lavoro di riferimento mostra che
...
Buona parte del lavoro è consistita nel ricostruire scelte che il lavoro di riferimento non
...
risultati molto più delle differenze rimaste, come mostrano i confronti raccolti nella
sezione~\ref{sec:risultati-sensibilita}, con scarti fino al 56\% sulla temperatura
...
trasformano il confronto con il lavoro di riferimento da una lettura a occhio in una misura.
...
\subsection{Da una cella a una griglia}
\label{sec:discussione-multicella}

I risultati presentati qui vengono da una sola cella e senza flusso. Una
simulazione vera ha molte celle e il gas passa dall'una all'altra: servono tre
aggiunte. La prima riguarda l'equazione dell'energia vibro-elettronica, che
oggi contiene solo le sorgenti: vanno aggiunti il trasporto, costruito con gli
stessi flussi alle facce già usati per le specie, e la conduzione. La seconda
sono le condizioni al contorno per i due campi nuovi: la temperatura
vibro-elettronica è letta dal caso e le ha già, mentre l'energia
vibro-elettronica viene ricavata dalla temperatura solo all'interno del
dominio, quindi i suoi valori sulle facce di bordo vanno ottenuti allo stesso
modo; pareti e ingressi hanno inoltre bisogno di una regola per la temperatura
vibro-elettronica, che un bagno termico chiuso non richiede. La terza riguarda
la densità usata nelle sorgenti: oggi è quella lasciata dall'aggiornamento
precedente della pressione, il che è esatto a volume costante, ma quando il
gas può comprimersi va sostituita con quella del passo corrente.

Per il resto non ci sono ostacoli. Ogni grandezza è calcolata cella per cella:
nessuna chiamata alla libreria ha bisogno di informazioni dalle celle vicine e
nessuna richiede somme su tutto il dominio. La suddivisione del dominio già
usata da OpenFOAM per il calcolo in parallelo vale quindi così com'è, e il
costo cresce in proporzione al numero di celle. Il primo caso di prova
naturale è il tubo d'urto monodimensionale già presente nel progetto.
