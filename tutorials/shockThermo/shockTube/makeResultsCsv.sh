...
if [ ! -d "$probeRoot" ]; then
    echo "Error: directory $probeRoot not found. Run foamRun first." >&2
    exit 1
fi
...
if [ -z "$probeTimeDir" ]; then
    echo "Error: no probe output in $probeRoot." >&2
    exit 1
fi

for field in p T Tve rho; do
    if [ ! -f "$probeTimeDir/$field" ]; then
        echo "Error: file $probeTimeDir/$field not found." >&2
        exit 1
    fi
done
...
echo "Created results.csv from $probeTimeDir"
